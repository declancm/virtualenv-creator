...
while True:
    ...
    try:
        ...
        continue
    confirmation = input(
        "\nAre you sure you want to install for the shell '{}' ? (y/n) : ".format(
            shellList[shellNumber]
        )
    )
    if confirmation == "y":
...
if shell == "ZSH":
    ...
    profileLocation = expandPath(
        "%USERPROFILE%\\Documents\\PowerShell\\Microsoft.PowerShell_profile.ps1"
    )
    alias = 'function runPyvenv {{ Invoke-Expression "pwsh /nologo -ExecutionPolicy Bypass -NoExit -File {}\\src\\PowerShell\\pyvenv.ps1" }}; Set-Alias pyvenv runPyvenv'.format(
        projectPath
    )
    addAlias(alias, profileLocation)
    exit(
    ...
    alias = 'function runPyvenv {{ Invoke-Expression "PowerShell.exe /nologo -ExecutionPolicy Bypass -NoExit -File {}\\src\\PowerShell\\pyvenv.ps1" }}; Set-Alias pyvenv runPyvenv'.format(
        projectPath
    )
    addAlias(alias, profileLocation)
    exit(
