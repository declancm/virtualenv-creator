...
if ($setAlias -eq 'y' -or $setAlias -eq 'yes') {
  ...
  if (Test-Path $location) {
    if (Select-String -Path $location -Pattern "Set-Alias pyvenv runPyvenv" -SimpleMatch -Quiet) {
      ...
      Add-Content $location "`nfunction runPyvenv { Invoke-Expression `"pwsh /nologo -ExecutionPolicy Bypass -NoExit -File $HOME\Documents\virtualenv-creator\pyvenv-PS.ps1`" }`nSet-Alias pyvenv runPyvenv"
      "The alias was added. Please restart PowerShell for the alias to take effect.`n"
    }
  } else {
    New-Item -Path $location -Type File -Force -Value "function runPyvenv { Invoke-Expression `"pwsh /nologo -ExecutionPolicy Bypass -NoExit -File $HOME\Documents\virtualenv-creator\pyvenv-PS.ps1`" }`nSet-Alias pyvenv runPyvenv" | Out-Null
